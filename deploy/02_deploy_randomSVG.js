...
module.exports = async ({
    ...
    if (chainId != 31337) {
        ...
        let vrf_tx = await vrfCoordinator.callBackWithRandomness(receipt.logs[3].topics[1], 2323239048, randomSVG.address)
        await vrf_tx.wait(1)
        log(`Now let's finsih the mint...`)
        let finish_tx= await randomSVG.finishMint(tokenId, { gasLimit: 30000000 })
        await finish_tx.wait(1)
        log(`You can view the tokenURI here: ${await randomSVG.tokenURI(tokenId)}`)
    }
}
